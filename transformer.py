...
with open('tadeusz.txt', 'r', encoding='utf-8') as f:
    text = f.read()
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        outs = [h(x) for h in self.heads] # (B,T,C)
        out = torch.cat(outs, dim=-1) # (B,T,C*n_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, n_embd, n_heads):
        ...
        self.sa = MultiHeadAttention(n_heads=n_heads, head_size=head_size, n_embd=n_embd)
...
class TransformerLanguageModel(nn.Module):
    ...
    def __init__(self, vocab_size, n_embed):
        ...
        self.blocks = nn.Sequential(
            Block(n_embd=n_embed, n_heads=6),
            Block(n_embd=n_embed, n_heads=6),
            Block(n_embd=n_embed, n_heads=6),
            #Block(n_embd=n_embed, n_heads=6),
            #Block(n_embd=n_embed, n_heads=6),
